...
| R7 — 媒体打开与播放列表 | In Progress | **R7-01 / R7-02 Complete**；R7-02 完整门禁 **83/83 CTest PASS（70.87 s）**。**R7-03 URL Open implementation candidate** style 修复后 Windows Quick **79/79 PASS（36.44 s）**、完整 **86/86 PASS（71.04 s）**，启动期 native Controls warning 已消失；实机继续暴露远程媒体加载悬挂后无法返回的恢复缺口，当前修复候选已补 Loading Cancel、LoadMedia timeout 与 Error recovery，更新后的 Quick **80/80**、完整 **87/87** 和实机恢复路径待复验 |
...
- style 修复后用户在锁定 Windows 环境完成 Debug build、Quick **79/79 PASS（36.44 s）** 与完整 **86/86 PASS（71.04 s）**；两次 `Player.exe` 运行均正常 exit code 0，先前 `TextField.background` / native Controls style warning 已不再出现。但真实 URL 手工验证继续暴露：有效 HTTP/HTTPS MP4 地址如果 backend 长时间无法完成加载，Snapshot 会卡在 Opening，用户只能重启。根因是既有 30 s `RequestTimeoutMonitor` 只在 `RequestTracker` 内将过期请求标记 Timeout，没有把超时 request type / generation 回传给 Session，因此不会产生 `MediaFailedEvent`；同时 Loading 状态没有取消入口。当前修复让 Tracker 返回完整过期 request record，Monitor 回调 Session，仅对**当前 MediaGeneration + LoadMedia + Opening**的超时提交 Media failure 并关闭 generation gate；正常 Stop/Shutdown 同样关闭旧 generation，避免迟到媒体事件重新污染状态。Loading 增加 `Cancel`，严格复用既有 `PlayerTransportViewModel.requestStop() → TransportAction::Stop → Empty` 主链；Error 增加 `Open media` recovery，不建立第二套 Playback 状态或命令。新增 `playback_request_timeout_recovery`，并扩展 `player_url_open` / `player_status_overlay` 覆盖 timeout metadata、Cancel 与 Error recovery；未新增生产依赖。更新后的 Windows build、Quick **80/80**、完整 **87/87** 以及“Loading Cancel → Empty → 再打开媒体 / timeout → Error → Open media”的实机链尚待复验，因此 **R7-03 保持 implementation candidate**。
