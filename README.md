...
| R7 — 媒体打开与播放列表 | In Progress | **R7-01 Local File Open second Windows revalidation reached 80/81 CTest**；Debug build **278/278 PASS**、QML lint **6/6**；唯一失败为 R7 Empty 状态引入后旧 Status ViewModel signal 测试契约，测试修复已提交；**81/81 + Player.exe/runtime/manual media** 复验待执行 |
...
- 自审发现仅完成 load 提交会导致 R6-02 `VideoViewport.hasMedia/hasVideo` 仍停留在占位值、视频可能后台播放但画面隐藏，因此新增职责独立的 `PlayerMediaViewModel`：只读消费同一 `PlaybackSnapshot`，仅在 Ready/Ended 且 source 已建立时投影 `hasMedia`，`hasVideo` 只读取 `snapshot.streams().video`。`ApplicationBootstrap → MainWindow → PlayerScreen → VideoViewport` 显式传递该投影，不建立第二份播放状态真值。
...
- 修复严格限定在既有 `player_status_view_model` 测试：新增 `Empty → Ready/None` 必须恰好发一次状态变化的直接断言，再在已经进入 Ready/None 后验证 `Playing ↔ Idle` 等不改变 Status 的快照不重复发信号；生产 `PlayerStatusViewModel`、selector、Empty Overlay 与 PlaybackSnapshot 均未改动，也没有放宽测试。CTest target 数仍为 **81**。由于第二轮 `test.ps1` 在 80/81 处以 exit code 8 阻断，`Player.exe` startup/runtime、真实本地视频打开、中文/空格路径以及播放中关闭的手工验收均未执行，**R7-01 仍不是 Complete，Stage R7 仍为 In Progress**；最终 **81/81 + runtime/manual media** 复验待执行。
