...
| R8 — 音轨、字幕、章节 | In Progress | **R8-01 ~ R8-12 Complete**。R8-12 将 chapter start 的 finite/non-negative/duration-bound 校验收口到 Playback Chapter domain，ChapterModel 只投影已验证行及 `normalizedTime`；Chapter QML 只发统一 absolute seek intent，Timeline marker 只读 model role，不再在 QML 修正越界数据，position 仍只来自 generation-gated Snapshot。Windows build 已完成，Quick **106/106 PASS（49.22 s）**、Full **114/114 PASS（79.19 s）**；startup-smoke **3.06 s**，8 项 windowed-render 合计 **39.29 s**。Stage R8 首轮联合真实媒体 smoke 的自动门禁拒绝了无进程/日志证据的人工 PASS，并暴露 Chapter delegate 必填 role 遮蔽；产品修复在 `87c4959` 完成 Windows configure/build，但新增 runtime 回归因未把 ListView 挂入可见 `QQuickWindow` 而使 Quick 为 **105/106 PASS**。测试夹具修复候选仍待 Windows Quick/Full 与完整联合 smoke 复验 |
...
- 用户在 HEAD `87c4959e0e6b8be057bd39879b56a1537e9168cd` 完成 Windows configure/build；Quick **105/106 PASS**，唯一失败为新增 runtime 回归在脱离 `QQuickWindow` 的根 Item 上等待惰性 ListView delegate，实际行数保持 0。输出没有再次出现原始 `title/timeText` required-property 警告，仅有锁定测试环境既有的 Qt font-directory QWARN。现将 `ChapterContent` 挂入可见 offscreen `QQuickWindow` 后再断言 delegate role 与 QML warning；新候选仍待 Windows Quick/Full、startup/windowed-render 与联合真实媒体复验，Stage R8 继续保持 In Progress。
