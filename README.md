...
| R7 — 媒体打开与播放列表 | In Progress | **R7-01 Local File Open candidate 修复复验待执行**；Windows configure **PASS（4.9 s / 2.6 s）**；首轮 Debug build 因 `PlayerMediaViewModel` 对 `std::optional<QString>` source 的类型误用 **FAILED**，修复已提交；build/QML lint/**81-test**/runtime validation pending |
...
- 已修复上述确定性类型错误：`PlayerMediaViewModel` 先读取 `source.has_value()`，再对内部 `QString` 调用 `isEmpty()`；并在既有 `player_media_view_model` target 增加 `Ready + video stream + missing source` 回归，要求仍保持 `hasMedia=false / hasVideo=false`，防止只修编译而破坏 projection 语义。没有修改 `PlaybackMediaState`、`PlaybackSnapshot`、公共播放数据格式或 CTest target 数，因此复验目标仍是 **81 项**。修复后的 Debug build、QML lint、**81/81 CTest**、`Player.exe` startup/runtime 与真实视频文件手工打开仍待 Windows 复验，**R7-01 不是 Complete，Stage R7 仍为 In Progress**。本轮没有实现 Playlist Domain、拖放、URL、argv、自动下一项或 R7-02 之后任务；没有修改 Persistence、配置或 `MpvVideoRenderer`/RenderContext 实现。
