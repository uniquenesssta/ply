...
| R8 — 音轨、字幕、章节 | In Progress | **R8-01 ~ R8-12 Complete**。R8-12 将 chapter start 的 finite/non-negative/duration-bound 校验收口到 Playback Chapter domain，ChapterModel 只投影已验证行及 `normalizedTime`；Chapter QML 只发统一 absolute seek intent，Timeline marker 只读 model role，不再在 QML 修正越界数据，position 仍只来自 generation-gated Snapshot。Windows build 已完成，Quick **106/106 PASS（49.22 s）**、Full **114/114 PASS（79.19 s）**；startup-smoke **3.06 s**，8 项 windowed-render 合计 **39.29 s**。Stage R8 首轮联合真实媒体 smoke 的自动门禁拒绝了无进程/日志证据的人工 PASS，并暴露 Chapter delegate 必填 role 遮蔽；产品修复已完成，新增 runtime 回归则连续两次因测试依赖 ListView 的惰性可视化时机而使 Quick 为 **105/106 PASS**。现改用同步 Instantiator 直接验证同一 required-role delegate 契约；新候选仍待 Windows Quick/Full 与完整联合 smoke 复验 |
