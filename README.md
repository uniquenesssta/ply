...
| R7 — 媒体打开与播放列表 | In Progress | **R7-01 / R7-02 / R7-03 / R7-04 / R7-05 / R7-06 Complete**；R7-07 已按实际 Figma canonical `4:2 / 4:48 / 4:88` 重新实现 Main Player / Fullscreen / Playlist Inspector 的 presentation，但本轮 Windows build / CTest / 实机视觉复核尚未执行，因此 **R7-07 仍未收口**。R7-06 既有 Windows Debug configure/build PASS、Quick **86/86 PASS**、完整 **93/93 CTest PASS（73.31 s）** 仍只代表上一已关闭 Atomic Task |
...
### 2026-08-17 — Canonical Figma UI reimplementation (validation pending)

- 重新读取实际 Figma `KIOxfwTvQJlcVLinkeJAxY` 的三个当前 canonical runtime frame：`4:2 Framework / Main Player`、`4:48 Framework / Fullscreen`、`4:88 Framework / Playlist Inspector`，不再以任务书摘要或旧候选 QML 近似替代最终视觉真值。Main Player 统一为 1320×700 reference surface；Floating Header 改为 Figma 的单一 500×54 glass capsule，直接显示真实媒体 title / technical metadata / PLAYING-PAUSED status 与唯一 Close visual；已被完整替代的 `MediaInfoPod.qml` / `WindowActionsPod.qml` 删除，避免保留重复 Header 路径。
- OSC 按 canonical state 重新落位：Main Standard 为 880×124；Playlist Inspector 打开时收为 730×124 并向左偏移到 reference x=110，为右侧 Inspector 腾出真实空间；Fullscreen 继续使用 828×106 compact geometry。Utility 顺序固定为 Subtitles / Playlist / Fullscreen，Subtitles 在 R8 真实功能实现前只提供无命令的 canonical visual placeholder，没有伪造 track action。
- Playlist Inspector 按 Figma 4:88 重建为 368×652 floating surface：标题/媒体数、324×42 Search、332×58 rows、Selected/Current 分层、3×32 Playing Rail 与 324×54 Footer 均使用 semantic token；新增独立 screen-local `PlaylistSearchField` / `PlaylistFooter`，Feature 层继续只允许直接 import `Theme + Controls`。Search 做本地只读过滤；双击播放、删除非 current、拖放重排全部通过现有 `PlaylistController`，没有绕过 readonly model 或建立第二套 Playlist/Playback state owner。PlaylistListModel 只新增只读 `currentPosition` 投影供 Footer 显示 current/count。
- `PlayerMediaViewModel` 新增只读 `title` / `metadataText`，仅从现有 `PlaybackSnapshot` 计算；媒体标题优先使用实际 title，缺失时回退 source/path 文件名；技术信息只显示真实可得的视频尺寸/像素格式，不伪造 codec/HDR。没有新增生产依赖，PlaybackSession、MediaGeneration、Render 生命周期、Playlist Domain/current owner 与现有 media-open 主链保持不变。
- 静态审查中发现新 Search/Footer 若直接放在 `features/playlist` 会违反 R5-01 的 Feature import gate，因此已经移动到 `screens/player/drawers`，没有放宽 `qml_module_boundaries`。本轮还同步补强了 canonical Header/Playlist/Media VM 定向测试、QML module 注册和 obsolete-file guard；测试没有被删除、skip 或弱化。
- 当前只完成了 Figma design-context 对照、源码/依赖方向/静态 contract 审查；这些新提交之后的 Windows `configure.ps1`、Debug `build.ps1`、Quick/Full CTest 与 `Player.exe` 三态视觉复核**尚未执行**，因此本条明确保持 validation pending，**R7-07 不标记 Complete**。已知仍未伪装完成的视觉/数据限制：R5-08 generic `Panel` 目前仍没有真实 ancestor backdrop capture blur；Playlist Domain 没有 queued per-entry duration，因此列表第二行继续显示真实 sourceLocation 而不是虚构 Figma 示例时长；R10 负责的 native frameless/hit-test/DWM/Snap 仍未提前实现。下一步必须先在锁定 Windows 环境完成 build/test，再以 Main / Fullscreen / Playlist Inspector 实机截图逐项与 Figma 对照。
...
- Fullscreen Header 在当前无媒体 identity 时不渲染空玻璃块；真正媒体标题绑定仍由后续媒体入口/presentation 数据链提供。R6-08 没有复制 Timeline/Transport/Volume：`PlayerScreen` 仍组合现有 `PlayerOscLayout` 与同一三类 Feature，只把 `compact` 的模式来源改为 `fullScreen`。`PlayerOscLayout/OscSurface` 在 Fullscreen 使用 828px max width；`TransportControls` 在 compact 下复用同一 intent/VM，但把 Play 从 40px Primary glass 降为 Secondary，并用新增的通用 `IconButton.iconSizeOverride` 输出 21px visual；32px Secondary hit target 继续保持。`VolumeControls` 在 compact 下继续隐藏 Slider 并把 Volume visual 降为 21px。
