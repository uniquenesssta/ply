...
- 用户先后在 HEAD `87c4959e0e6b8be057bd39879b56a1537e9168cd` 与 `c8574f49adb8a8cc763871c0077cc6aedcc62a4a` 完成 Windows configure/build；两轮 Quick 均为 **105/106 PASS**，唯一失败都是测试未创建 ListView delegate，第二轮即使挂入可见 `QQuickWindow` 仍保持 0 行。两轮均未再次出现原始 `title/timeText` required-property 警告，仅有锁定测试环境既有的 Qt font-directory QWARN。现以同步 `Instantiator` 复用生产 `ChapterRow` delegate 形状，直接验证 C++ model 的 `index/title/timeText` required-role 注入，并保留生产 `ChapterContent` 不得重声明 role 的静态检查；新候选仍待 Windows Quick/Full、startup/windowed-render 与联合真实媒体复验，Stage R8 继续保持 In Progress。
- 用户在 HEAD `d6194d667bbb2b3f5fe1523b2c05cb352497506c` 执行修订后的联合真实媒体 smoke：R8-S1 ~ R8-S10 均手工输入 PASS，Player 以 0 正常退出，主媒体加载记录为 2；但自动门禁只观测到外挂字幕 submitted=1 / deduplicated=0，且 cleanup media 加载为 0，因此最终结果保持 FAIL。控制台同时显示 S2/S5/S8 等中文说明在弯引号处截断，确认是 Windows PowerShell 将弯引号解释为字符串边界的验证包缺陷，而非足以判定产品失败的证据；修订包改用方头括号并把 S2/S5/S8/S9/S10 拆为短行，S5 明示四次文件选择、S9 明示两次拖放，自动日志门禁保持不放宽。
