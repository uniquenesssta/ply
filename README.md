...
| R7 — 媒体打开与播放列表 | In Progress | **R7-01 Local File Open video-output repair candidate**；第二轮 Windows Debug build **278/278 PASS**、QML lint **6/6**、CTest **80/81**；随后实机本地视频确认 load/audio/controls 正常但画面未显示，video capability projection 修复已提交；**81/81 + 可见视频画面**复验待执行 |
...
- 自审发现仅完成 load 提交会导致 R6-02 `VideoViewport.hasMedia/hasVideo` 仍停留在占位值、视频可能后台播放但画面隐藏，因此新增职责独立的 `PlayerMediaViewModel`：只读消费同一 `PlaybackSnapshot`，仅在 Ready/Ended 且 source 已建立时投影 `hasMedia`。初始 candidate 曾错误以 `snapshot.streams().video` 的运行时 `video-params` 作为 `hasVideo` 启动门槛；实机“有声无画面”后已收敛为读取 reducer 从 `track-list` 维护的 `snapshot.capabilities().hasVideoTrack`，而 `streams.video` 继续只表达分辨率/像素格式/旋转等视频参数。`ApplicationBootstrap → MainWindow → PlayerScreen → VideoViewport` 显式传递该投影，不建立第二份播放状态真值。
- 进一步主链审计确认产品代码此前没有把 PlaybackSession 所拥有的 libmpv core 绑定给 R4 `MpvVideoItem`，因此仅显示 Viewport 仍会出现“后台已加载但视频无画面”。新增职责独立的 `PlayerVideoRenderBinding` 作为 composition bridge：Playback backend 只向上发布不透明 `quintptr` core address，`src/app` 不接触 `mpv_handle` C API 类型；真正地址→`mpv_handle*` 转换仍在 mpv infrastructure 的 `MpvVideoItem::setRenderCoreAddress()` 内完成。Binding 不拥有 mpv core、`MpvRenderContext` 或播放状态，只绑定现有 `mpvVideoItem` 并复用 R4 `MpvRenderShutdownCoordinator`。关闭顺序固定为 `beginRenderShutdown → destroy QML/scene graph → wait/prove render release → stop PlaybackSession/core`；若 render release 未能证明则阻断不安全 core teardown。`MpvVideoRenderer` 与 RenderContext 实现没有改写。
- 新增独立 `media_open_coordinator`、`media_open_playback_integration`、`player_media_open`、`player_media_view_model`、`player_video_render_binding` 五个 CTest target。覆盖 cancel/no-op、非本地、缺失、目录、空格/Unicode canonical path、submission rejection；真实 integration 使用运行时生成的短 WAV 走 `MediaOpenCoordinator → PlaybackCommandBus → PlaybackSessionThread → libmpv`；Presentation contract 锁定 picker/Coordinator/Validator/Viewport/opaque render bridge 与 shutdown order。重新 configure 后预计全量测试数 **76 → 81**。
- 首轮 Windows 锁定环境验证：`configure.ps1` **PASS**（Configuring **4.9 s** / Generating **2.6 s**）。Debug `build.ps1` 在 `player_presentation_media` 编译阶段 **FAILED**：`PlayerMediaViewModel` 将 `PlaybackMediaState::source` 的真实类型 `std::optional<QString>` 当成 `QString` 直接调用 `.isEmpty()`，MSVC 报 C2039，并连带触发 `nextHasMedia` 的 C2737。该轮 build 未完成，因此 QML lint、81 项 CTest、`Player.exe` runtime 和真实视频手工打开均没有执行，不计为通过；验证前本地受保护 `.gitignore` 修改与 `r4-04-qml-diagnostics/` 未跟踪目录仍存在。
- 已修复上述确定性类型错误：`PlayerMediaViewModel` 先读取 `source.has_value()`，再对内部 `QString` 调用 `isEmpty()`；并在既有 `player_media_view_model` target 增加 `Ready + video stream + missing source` 回归，要求仍保持 `hasMedia=false / hasVideo=false`，防止只修编译而破坏 projection 语义。没有修改 `PlaybackMediaState`、`PlaybackSnapshot`、公共播放数据格式或 CTest target 数，因此复验目标仍是 **81 项**。
- 第二轮 Windows 锁定环境复验：Debug `build.ps1` 完成 **278/278 PASS**，development runtime root marker 与 Qt runtime deployment 均 PASS；`scripts/test.ps1` 完成 **6/6 QML lint**，全量 CTest 为 **80/81 PASS、1 failed，75.67 s**。唯一失败是 `player_status_view_model::irrelevantSnapshotDoesNotEmit`：R7-01 已将 ViewModel 初始状态从 R6 的 None 改为默认 Snapshot 所对应的 Empty，因此首次收到 `Ready + non-buffering` 快照时发生真实 **Empty → None** UI 状态变化，必须发出 `stateChanged()` 以撤掉 Empty Overlay；旧测试仍要求首次 Ready 不发信号，测试契约已经过期。该轮 `player_media_view_model` **0.10 s PASS**，确认 optional source 修复已经通过真实 Windows 编译/测试。既有 Qt 6.8.3 `qjsengine.h/qvariant.h/qjsprimitivevalue.h` C4702 system-header warning 仍为已记录外部工具链 warning，没有形成 build/test 主因。
- 修复严格限定在既有 `player_status_view_model` 测试：新增 `Empty → Ready/None` 必须恰好发一次状态变化的直接断言，再在已经进入 Ready/None 后验证 `Playing ↔ Idle` 等不改变 Status 的快照不重复发信号；生产 `PlayerStatusViewModel`、selector、Empty Overlay 与 PlaybackSnapshot 均未改动，也没有放宽测试。CTest target 数仍为 **81**。由于第二轮 `test.ps1` 在 80/81 处以 exit code 8 阻断，该轮没有继续做产品 runtime；后续用户单独启动产品完成了真实视频检查。
- 后续实机本地视频验证确认：媒体可以正常打开并播放，音频、播放/暂停、Timeline、音量等主链均正常，但视频区域**没有画面**。用户提供的 `player-20260816-004147.log` 显示 libmpv **0.41.0 / FFmpeg 8.0.3** 初始化正常、Desktop OpenGL **4.6 / NVIDIA RTX 3070** backend validation 正常，运行过程中未记录 QML/runtime/render-context failure，并以 exit code **0** 正常结束。结合产品 QML `VideoSurface.visible = hasMedia && hasVideo` 与 Domain 已有 `track-list → capabilities.hasVideoTrack`，根因收敛为 Presentation 将 `video-params` (`snapshot.streams().video`) 错当成视频输出启动能力，形成“等待运行时视频参数才显示 Render Surface”的错误门槛。修复仅将 `PlayerMediaViewModel::hasVideo` 改为 `hasMedia && snapshot.capabilities().hasVideoTrack`；不修改 reducer、track-list、libmpv 初始化、Render context、公共 PlaybackSnapshot 结构或音频行为。定向测试新增“有 video track capability、尚无 video-params 时仍必须打开 VideoViewport”以及“只有 video-params、没有 video track capability 时不得冒充视频媒体”。该修复后的 Windows build/QML lint/**81/81 CTest** 与真实可见视频画面仍待复验，**R7-01 不是 Complete，Stage R7 仍为 In Progress**；本轮没有实现 Playlist Domain、拖放、URL、argv、自动下一项或 R7-02 之后任务。
...
- 新增 `player_fullscreen_controls` CTest，并同步修正 R6-04 Bottom Region contract：`compact` 明确代表 Fullscreen mode，而不是普通 Main Window 宽度断点。Qt 6.8 API 静态核对确认 `Shortcut/Qt.ApplicationShortcut`、`TapHandler.doubleTapped` 与 `Window.showFullScreen()/showMaximized()/showNormal()` 均属于锁定版本支持能力。
...
- 新增 `player_volume_view_model`、`player_volume_controls` 两个 CTest target，并扩展 `application_container` 的直接 composition 链接/生命周期边界；全量测试数 **62 → 64**。
